...
private:
	...
	std::vector<pcl::PointCloud<PointT>> map_clouds;
...
void InstanceBeliefUpdate<PointT>::setInputCloud(const std::vector<pcl::PointCloud<PointT>>& clouds) 
{
	...
	nums.resize(window_size);
	ins_clouds.resize(window_size);
	map_clouds.resize(window_size);
	...
	for (int i = 0; i < window_size; ++i) 
	{
		auto& cloud = map_clouds[i];

		for (int j = 0; j < window_size; ++j) 
		{
			auto& ins_cloud = ins_clouds[j];
...
void InstanceBeliefUpdate<PointT>::updateBelief() 
{
	...
	for (int i = 0; i < window_size; ++i) 
	{
		if (0 < nums[i]) 
		{
			...
			// printf("ins grid data  num: %ld\n", ins_data.size());

			auto& map_cloud = map_clouds[i];
			std::unordered_map<int, float> map_data;
			...
			for (std::set<int>::iterator iter = grid_index.begin(); iter != grid_index.end(); ++iter) 
			{
				...
				grid_data[index] += update_log_odd;
				// printf("ind_map_ins_rat_odd(%d, %f, %f, %f, %f)\n", index, mv, iv, ratio, update_log_odd);
			}
		}
	}

	for (auto iter = grid_data.begin(); iter != grid_data.end(); ++iter) 
	{
		int index = iter->first;
		float grid_log_odd = iter->second;
		log_odd += grid_log_odd;
		// printf("grid_log_odd: %f\n", grid_log_odd);
	}

	// printf("log_odd: %f, pro: %f\n", log_odd, logistic(log_odd));

	log_odd = log_odd/(float)grid_data.size();

	// printf("mean log_odd: %f, pro: %f\n", log_odd, logistic(log_odd));

	// printf("log_odd: %f, pro: %f\n", log_odd, logistic(log_odd));
	// printf("mean log_odd: %f, pro: %f\n", log_odd/(float)grid_data.size(), logistic(log_odd/(float)grid_data.size()));
	// for (int i = 0; i < window_size; ++i) 
	// {
...
jsk_recognition_msgs::PolygonArray InstanceBeliefUpdate<PointT>::getGridMap(const std_msgs::Header& header) 
{
	...
	for (int l = 0; l < length; ++l) 
	{
		for (int w = 0; w < width; ++w)
		{
			...
			for (int i = 1; i <= 4; ++i) 
			{
				angle = angle + (grid_azimuth / 4.0f) / 180.0f * M_PI;
				point.x = range * cos(angle);
				point.y = range * sin(angle);
				point.z = lower_height;
				polygon.polygon.points.push_back(point);
	...
	for (auto iter = grid_data.begin(); iter != grid_data.end(); ++iter) 
	{
		int index = iter->first;
		float grid_log_odd = iter->second;
		grid_map.likelihood[index] = logistic(grid_log_odd);
		// printf("grid_log_odd: %f\n", logistic(grid_log_odd));
	}
